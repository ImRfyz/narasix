...
if ( ! class_exists( 'Narasix_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 ...
	 * @since Narasix 1.0
	 */
	class Narasix_Walker_Page extends Walker_Page {

		/**
		 * Outputs the beginning of the current element in the tree.
		 *
		 * @since Narasix 1.0
		 * @since Narasix 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'narasix' ), $page->ID );
			}
			...
			if ( isset( $args['show_toggles'] ) && true === $args['show_toggles'] ) {
				...
				if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
					...
					$toggle_duration      = narasix_toggle_duration();

					// Add the sub menu toggle.
					$args['list_item_after'] .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'narasix' ) . '</span>' . narasix_get_theme_svg( 'chevron-down' ) . '</button>';
