 ...
 */
	...
	$narasix_post_title    = get_theme_mod( 'narasix_single_blog_post_title_tag', true );
	$narasix_author_meta   = get_theme_mod( 'narasix_single_blog_post_author', true );
...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'wysiwyg wysiwyg-slate max-w-full space-y-8 dark:wysiwyg-invert' ); ?>>
  ...
  <?php if ( has_post_thumbnail() ) : ?>
    <div class="relative not-wysiwyg h-[16rem] sm:h-[24rem] w-full overflow-hidden rounded-lg">
    ...
    <?php if ( ! empty( $narasix_blog_post_tag ) ) { ?> 
        ...
        <div class="clear-both not-wysiwyg space-x-2 border-y border-dashed py-6 dark:border-charcoal-700">
          <ul class="flex flex-wrap items-center ">
            <span class="text-base font-bold text-slate-800 mr-2"><?php esc_html_e( 'Tags:', 'narasix' ); ?></span>
            <?php
              foreach( $post_tags as $post_tag ) : ?>
              <li class="inline-flex items-center">
                <a href="<?php echo esc_url( get_tag_link( $post_tag->term_id ) ); ?>" class="text-[13px] px-3 py-1 no-underline dark:bg-charcoal-700 bg-white hover:scale-105 rounded-lg" rel="noopener noreferrer">#<?php echo esc_html( $post_tag->name ); ?></a>
              </li>
            <?php endforeach; ?>
          </ul>
        </div>
